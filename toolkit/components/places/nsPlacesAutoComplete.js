...
nsPlacesAutoComplete.prototype = {
                                        ...
                                        aPreviousResult, aListener)
  {
    ...
    // Unlike nsURLInlineComplete, we don't want this search to start
    // synchronously. Wait 50ms before launching the query.
    this._startTimer = Cc["@mozilla.org/timer;1"]
                       .createInstance(Ci.nsITimer);
    let self = this;
    let timerCallback = function() {
      self._doStartSearch(aSearchString, aSearchParam,
                          aPreviousResult, aListener);
    };
    this._startTimer.initWithCallback(timerCallback, 50, Ci.nsITimer.TYPE_ONE_SHOT);
  },
...
function nsURLInlineComplete()
{
  ...
  XPCOMUtils.defineLazyGetter(this, "_syncQuery", function() {
    ...
    // Do not warn about the sorting, as id is a primary key so it
    // has an implicit index.
    return this._db.createStatement(
      "/* do not warn (bug 566489) */"
    + "SELECT host || '/' "
    + "FROM moz_hostnames_temp "
...
nsURLInlineComplete.prototype = {
  ...
  _initDomainTable: function UIC_initDomainTable()
  {
    if (this._launchedInitQuery) {
      return;
    }

    // Use the main connection so we can use triggers on
    // moz_places.
    ...
    // Note: this should be kept up-to-date with the definition in
    //       nsPlacesTables.h.
    // Because we order by frecency in the query after this, then a
    // lower row id implies a higher frecency.
    ...
    // Trigger for adding a new page to moz_places.
    this._db.executeSimpleSQL(
      "CREATE TEMP TRIGGER moz_hostnames_temp_afterinsert_trigger "
    + "AFTER INSERT ON moz_places FOR EACH ROW "
    + "WHEN NEW.hidden = 0 AND NEW.rev_host != '' "
    + "BEGIN "
    +   "INSERT OR REPLACE INTO moz_hostnames_temp (id, host, page_count) "
    +   "VALUES ((SELECT id FROM moz_hostnames_temp WHERE host = fixup_url(get_unreversed_host(LOWER(NEW.rev_host)))), "
    +           "fixup_url(get_unreversed_host(LOWER(NEW.rev_host))), "
    +           "IFNULL((SELECT page_count FROM moz_hostnames_temp "
    +                   "WHERE host = fixup_url(get_unreversed_host(LOWER(NEW.rev_host)))), 0) + 1); "
    + "END "
    );

    // Trigger for deleting a page from moz_places.
    this._db.executeSimpleSQL(
      "CREATE TEMP TRIGGER moz_hostnames_temp_afterdelete_trigger "
    + "AFTER DELETE ON moz_places FOR EACH ROW "
    + "BEGIN "
    +   "UPDATE moz_hostnames_temp "
    +   "SET page_count = page_count - 1 "
    +   "WHERE host = fixup_url(get_unreversed_host(LOWER(OLD.rev_host))); "
    +   "DELETE FROM moz_hostnames_temp "
    +   "WHERE page_count = 0; "
    + "END "
    );
    ...
    let fillTableQuery = this._db.createAsyncStatement(
      "INSERT OR IGNORE INTO moz_hostnames_temp (host, page_count) "
    + "SELECT fixup_url(get_unreversed_host(LOWER(h.rev_host))) AS host, COUNT(*) as page_count "
    + "FROM moz_places h "
    + "WHERE h.hidden = 0 AND h.rev_host != '' "
    + "GROUP BY h.rev_host "
    + "ORDER BY MAX(h.frecency) DESC, MAX(h.id) DESC"
    );
