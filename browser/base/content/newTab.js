...
let NewTabPage = {
  ...
  getTopSites: function NTP_getTopSites() {
    ...
    NewTabUtils.getSites(function(sites) {
      ...
      for (let i = 0; i < sites.length; i++) {
        ...
        let siteItem = NewTabPage.createSiteItem(sites[i], i);
  ...
  createSiteItem: function NTP_createSiteItem(aSite, aSiteIndex) {
  ...
  removeSiteItem: function NTP_removeSiteItem(event) {
    let siteIndex = event.target.getAttribute("index");
    NewTabUtils.removeSite(siteIndex);

    let siteItem = document.querySelector(".site-item[index='" + siteIndex + "']");
