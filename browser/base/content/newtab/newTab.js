...
let Page = {
  ...
  _init: function Page__init() {
    ...
    addEventListener("unload", function () {
      NewTabUtils.Pages.unregister(this);
    }, false);
...
let Grid = {
  ...
  init: function Grid_init(aSelector) {
    ...
    // listen for 'visibilitychange' to redraw our grid
    document.addEventListener("mozvisibilitychange", function () {
      // we don't need to refresh if we're hidden
      if (!document.mozHidden && Page.isEnabled())
        self._refresh();
    }, false);

    // draw our grid the first time if we're visible
  ...
  reset: function Grid_reset() {
    ...
    Animations.fadeOut(this._node, function () {
  ...
  _refresh: function Grid__refresh() {
    // remove all sites
...
let Links = {
  ...
  _init: function Links__init() {
    let initStorageValues = this._initStorageValues.bind(this);

    // listen for 'visibilitychange' to re-read the values from storage
    document.addEventListener("mozvisibilitychange", initStorageValues, false);
