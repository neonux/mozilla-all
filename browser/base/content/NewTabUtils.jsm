...
// TODO
let NewTabUtils = {
  init: function NTU_init() {
    // prefetch the links to speed up rendering
    NewTabUtils.Links.fetchLinks(function () {});
  }
};
...
NewTabUtils.Storage = {
  ...
  set: function NTU_Storage_set(aName, aValue) {
    this._storage.setItem(aName, JSON.stringify(aValue));
  },

  clear: function NTU_Storage_clear() {
    this._storage.clear();
  }
};
...
NewTabUtils.Pages = {
  ...
  refresh: function NTU_Pages_refresh(aExcludePage) {
    this._pages.forEach(function (page) {
      if (page != aExcludePage)
        page.refresh();
    });
  }
};
