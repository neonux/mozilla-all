...
function gen_test()
{
  ...
  try {
    ...
    try {
      ...
      // We must specifically set focus on the first item in the list, if it is
      // not already focused, or the test might fail intermittently.
      if (document.commandDispatcher.focusedElement != richlistbox.firstChild)
      {
        richlistbox.addEventListener("focus", testRunner.continueTest, true);
        richlistbox.firstChild.focus();
        yield;
        richlistbox.removeEventListener("focus", testRunner.continueTest, true);
      }
