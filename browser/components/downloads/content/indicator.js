...
const DownloadsButton = {
  ...
  _updatePositionInternal: function DB_updatePositionInternal() {
    ...
    if ((!placeholder || !DownloadsIndicatorView.hasDownloads) &&
    ...
    if (placeholder) {
      ...
      if (isElementVisible(placeholder.parentNode)) {
        return DownloadsIndicatorView.indicatorIcon;
      }
    }

    // The indicator is normally invisible, do nothing unless we need an anchor.
    if (!this._anchorRequested) {
      return null;
    }
...
const DownloadsIndicatorView = {
  ...
  showEventNotification: function DIV_showEventNotification()
  {
    ...
    indicator.setAttribute("notification", "show");
    this._notificationTimeout = setTimeout(
         function() indicator.setAttribute("notification", "fadeout"), 3000);
  },
   ...
   * then the status portion of the indicator is not displayed.
   */
  set counter(aValue) {
    if (this._counter !== aValue) {
      ...
      this._indicatorProgress.parentNode.hidden = !aValue;
    }
    return aValue;
  },
  _counter: null,
  ...
  onClick: function DIV_onClick(aEvent)
  {
    // Only show the panel for left clicks.
    if (aEvent.button != 0) {
      return;
    }
...
XPCOMUtils.defineLazyGetter(DownloadsIndicatorView, "indicatorIcon",
                            function() document.getElementById(
