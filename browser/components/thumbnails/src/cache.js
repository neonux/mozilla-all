...
let Cache = {
  ...
  readEntry: function Cache_readEntry(aEntry, aCallback) {
  ...
  createWriteEntryTeeListener:
    function Cache_createWriteEntryTeeListener(aEntry, aListener) {

    let tee = Cc["@mozilla.org/network/stream-listener-tee;1"]
              .createInstance(Ci.nsIStreamListenerTee);

    let outputStream = aEntry.openOutputStream(0);
    tee.init(aListener, outputStream);

    return tee;
  },

  _isValidEntry: function Cache__isValidEntry(aEntry) {
    if (!aEntry)
      return false;
