...
// TODO
function ThumbnailProtocol() {
}

ThumbnailProtocol.prototype = {
  scheme: SCHEME,
  defaultPort: -1,
  ...
  newURI: function TP_newURI(aSpec, aOriginCharset, aBaseUri) {
    let uri = Cc["@mozilla.org/network/simple-uri;1"].createInstance(Ci.nsIURI);
    uri.spec = aSpec;
    return uri;
  },

  newChannel: function TP_newChannel(aUri) {
    let {url} = new URLParser(aUri).parse();
    ...
    return new RedirectChannel(aUri);
  },

  allowPort: function TP_allowPort(aPort, aScheme) {
    return false;
  },
...
let NSGetFactory = XPCOMUtils.generateNSGetFactory([ThumbnailProtocol]);
